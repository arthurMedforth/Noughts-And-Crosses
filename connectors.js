...
// Validate academy functions are available
...
function positionClick(rowIndex, columnIndex, event) {
    ...
    if (randomComputer===null&&flipRandom===null){
        ...
        if (winner) {
            if (typeof winner !== "string" || !["noughts", "crosses", "nobody",player1Name,player2Name,"Computer"].includes(winner)) {
            ...
            if (flipRandom){   
                const board = getBoard();
                let randomMoveIndices = getRandomMove(board)
                board[randomMoveIndices[0]][randomMoveIndices[1]]=turn
                takeTurn(randomMoveIndices[0], randomMoveIndices[1]);
                if (randomComputer){
                    setTimeout(function(){
                        drawBoard(board);
                        const winner = checkWinner();
